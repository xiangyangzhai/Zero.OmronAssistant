...
namespace Zero.CommunicationLib.Helper
{
    public  class ParityHelper
    {
        ...
        public static byte[] CalculateSUM(byte[] data)
        {
            ...
            return ByteArrayLib.GetAsciiByteArrayFromValue(((byte)sum));
        }
        ...
        {
            if (data.Length <= 2) return false;
            byte[] sum = CalculateSUM(ByteArrayLib.GetByteArrayFromByteArray(data, 0, data.Length - 2));
            return ByteArrayLib.GetByteArrayEquals(sum, ByteArrayLib.GetByteArrayFromByteArray(data, data.Length - 2, 2));
        }
    }
}
